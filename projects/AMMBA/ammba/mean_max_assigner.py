...
def mean_max_function(X, eps=1e-3):
    numerator = torch.sum(X / (1 - X + eps), dim=0)
    denominator = torch.sum(1 / (1 - X + eps), dim=0)
    result = numerator / denominator
    result = torch.clamp(result, min=0.005, max=0.5)
    return result
...
class MeanMaxAssigner(BaseAssigner):
    ...
    def assign(
        ...
        mean_max_pos_thr = mean_max_function(candidate_overlaps)
        is_pos = candidate_overlaps >= mean_max_pos_thr
