...
class STARMetric(BaseMetric):
    ...
    def __init__(self,
                 ...
                 ori_gt_ann_dir: str='data/STAR/test/object-TXT/') -> None:
        ...
        self.ori_gt_ann_dir = ori_gt_ann_dir
        if format_only == False:
            assert ori_gt_ann_dir is not None, \
                'For local original-image evaluation, ori_gt_ann_dir must be provided'
    ...
    def load_ori_gt_ann(self, img_name):
        """
        img_name: '0004', no suffix
        """
        logger = MMLogger.get_current_instance()
        gt_file = osp.join(self.ori_gt_ann_dir, f'{img_name}.txt')
        logger.info(f'Loading {gt_file}')
        ...
        for line in lines:
            ...
            label = self.dataset_meta['classes'].index(cls_name)

            if difficulty == 0:
                bboxes.append(coords)
                labels.append(label)
            else:
                bboxes_ignore.append(coords)
                labels_ignore.append(label)

        def _to_rbox(qboxes):
            if len(qboxes) == 0:
                return np.zeros((0, 5), dtype=np.float32)
            qboxes = torch.from_numpy(np.stack(qboxes, dtype=np.float32))
            rboxes = qbox2rbox(qboxes)  # (N, 5)
            return rboxes.numpy()

        if self.predict_box_type == 'rbox':
            bboxes = _to_rbox(bboxes)
            bboxes_ignore = _to_rbox(bboxes_ignore)
        elif self.predict_box_type == 'qbox':
    ...
    def merge_preds_to_ori(self, preds):
        """
        preds: list of pred dict (patch-level)
        return:
            ori_img_names, ori_dets
        """
        logger = MMLogger.get_current_instance()
        logger.info(f'[MergePred] patch preds: {len(preds)}')

        collector = defaultdict(list)

        for pred in preds:
            img_id = pred['img_id']
            oriname = img_id.split('__')[0]

            x, y = map(int, re.findall(r'\d+', re.findall(r'__\d+___\d+', img_id)[0]))

            bboxes = pred['bboxes'].copy()
            scores = pred['scores']
            labels = pred['labels']

            if self.predict_box_type == 'rbox':
                bboxes[:, :2] += np.array([x, y])
            else:  # qbox
                bboxes += np.array([x, y] * 4)

            dets = np.concatenate(
                [labels[:, None], bboxes, scores[:, None]],
                axis=1
            )
            collector[oriname].append(dets)

        ori_dets = []
        ori_names = []

        for oriname, det_list in collector.items():
            dets = np.concatenate(det_list, axis=0)
            labels, dets = dets[:, 0], dets[:, 1:]

            per_cls_dets = []
            for cls in range(len(self.dataset_meta['classes'])):
                cls_dets = dets[labels == cls]
                if len(cls_dets) == 0:
                    per_cls_dets.append(np.zeros((0, dets.shape[1])))
                    continue

                th = torch.from_numpy(cls_dets)
                if self.predict_box_type == 'rbox':
                    keep, _ = nms_rotated(th[:, :5], th[:, -1], self.iou_thr)
                else:
                    keep, _ = nms_quadri(th[:, :8], th[:, -1], self.iou_thr)

                per_cls_dets.append(keep.cpu().numpy())

            ori_names.append(oriname)
            ori_dets.append(per_cls_dets)

            total_before = dets.shape[0]
            total_after = sum(len(x) for x in per_cls_dets)
            logger.info(
                f'[MergePred] {oriname}: '
                f'before_nms={total_before}, after_nms={total_after}')

        logger.info(f'[MergePred] merged to {len(ori_names)} original images')

        return ori_names, ori_dets
    ...
    def compute_metrics(self, results: list) -> dict:
        ...
        if self.metric == 'mAP':
            ...
            ori_names, ori_dets = self.merge_preds_to_ori(preds)
            ori_gts = []
            for name in ori_names:
                gt = self.load_ori_gt_ann(name)
                ori_gts.append(gt)
            logger.info(f'[Eval] original images: {len(ori_names)}')
            logger.info(f'[Eval] GT loaded from: {self.ori_gt_ann_dir}')
